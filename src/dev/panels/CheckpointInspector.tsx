...
import type { Checkpoint, Bundle, Transition } from "@/aggregation/types";
    ...
    return (
        <div className="overflow-hidden rounded-md border">
            <div className="flex items-center gap-3 bg-muted/20 px-4 py-2 text-sm">
                ...
                {!isOpen && (
                    <span className="text-muted-foreground">#{String(index)}</span>
                )}
            ...
            <div className="space-y-1 border-t border-border/50 px-4 py-2">
                ...
                {bundle.endedAt && (
                    <Row
                        label="Duration"
                        value={formatDuration(bundle.endedAt - bundle.startedAt)}
                    />
                )}
                {bundle.text && (
...
function useCheckpointStorage() {
    ...
    useEffect(() => {
        ...
        const onChange = (
            ...
            setCp(changes.checkpoint.newValue ?? null);
        };
